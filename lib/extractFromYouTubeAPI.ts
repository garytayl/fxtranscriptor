...
export async function extractFromYouTubeAPI(
  ...
  try {
    ...
    // Note: captions.download requires OAuth2 for some videos, but may work with API key for public auto-generated captions
    ...
    try {
      ...
      console.error(`[YouTube API] Direct download failed: ${errorMsg}`);
      
      // If OAuth is required, we can't proceed without OAuth tokens
      // However, we can still provide helpful error message
      if (errorMsg.includes('OAuth') || errorMsg.includes('403')) {
        throw new Error(`Caption download requires OAuth2 authentication. This is a limitation of YouTube Data API v3 for serverless functions. Consider using Whisper AI fallback for videos without accessible captions.`);
      }
      throw downloadError;
    }
